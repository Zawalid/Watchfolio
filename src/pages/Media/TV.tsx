...
import { discoverTvShows, getDefaultSortForCategory, getFiltersForCategory, type DiscoverParams } from '@/lib/api/TMDB';
import { queryKeys } from '@/lib/react-query';

export default function Tv({ category }: { category: Categories }) {
  ...
  if (selectedNetworks?.length) {
    discoverParams.with_networks = selectedNetworks.join(',');
  }
  ...
  if (minYear !== null) {
    discoverParams['first_air_date.gte'] = `${minYear}-01-01`;
  }

  if (maxYear !== null) {
    discoverParams['first_air_date.lte'] = `${maxYear}-12-31`;
  }
  ...
  return (
    <MediaCardsList
      queryOptions={{
        queryKey: queryKeys.discover('tv', category!, discoverParams),
        queryFn: async () => await discoverTvShows(discoverParams),
        enabled: !!category,
