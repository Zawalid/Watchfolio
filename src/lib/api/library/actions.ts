...
export const getMyLibrary = (): UserMediaData[] => {
  const collection = getLibraryCollection();
  return Object.values(collection);
};

export const getItemFromLibrary = (mediaType: 'movie' | 'tv', id: number): UserMediaData | undefined => {
  const collection = getLibraryCollection();
...
export const upsertItemInLibrary = (
  ...
  const newItemData: UserMediaData = { ...defaultItemData, ...(existingItem || {}), ...itemProps };
...
export const setItemStatus = (mediaType: 'movie' | 'tv', id: number, status: UserMediaStatus): UserMediaData => {
  return upsertItemInLibrary({ id, mediaType, status });
};

export const toggleItemFavorite = (mediaType: 'movie' | 'tv', id: number): UserMediaData => {
...
export const setItemUserRating = (mediaType: 'movie' | 'tv', id: number, rating?: number): UserMediaData => {
  // rating can be undefined to clear it
  return upsertItemInLibrary({ id, mediaType, userRating: rating });
};

export const setItemNotes = (mediaType: 'movie' | 'tv', id: number, notes: string): UserMediaData => {
  return upsertItemInLibrary({ id, mediaType, notes });
};

export const addWatchDate = (mediaType: 'movie' | 'tv', id: number, date?: string): UserMediaData => {
...
export const updateTvShowProgress = (tvShowId: number, seasonNumber: number, episodeNumber: number): UserMediaData => {
  return upsertItemInLibrary({
    id: tvShowId,
    mediaType: 'tv',
    lastWatchedEpisode: {
